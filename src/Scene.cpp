...
std::vector<std::shared_ptr<Object>> Scene::InitObjects()
{
	...
	std::shared_ptr<Sphere> sphere5 = std::make_shared<Sphere>(0.7, Vector(floorPlane->GetCenter().x + 3, 0.7, floorPlane->GetCenter().z - 2));
...
std::vector<std::shared_ptr<Light>> Scene::InitLightSources()
{
	...
	std::shared_ptr<Light> light3 = std::make_shared<Light>(Vector(0, 3, 6), white, 1.4);
