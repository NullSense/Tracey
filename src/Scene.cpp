...
std::vector<std::shared_ptr<Object>> Scene::InitObjects()
{
	...
	behindPlane->SetMaterial(green);

	std::shared_ptr<Sphere> sphere1 = std::make_shared<Sphere>(0.5, Vector(0.5, 0.5, -2.5));
	sphere1->SetMaterial(waterM);
	std::shared_ptr<Sphere> sphere2 = std::make_shared<Sphere>(1.3, Vector(-1, 0.3, -4));
	sphere2->SetMaterial(waterM);
	...
	sphere3->SetMaterial(waterM);
	std::shared_ptr<Sphere> sphere4 = std::make_shared<Sphere>(1, Vector(sphere1->GetCenter().x - 2, sphere1->GetCenter().y + 3, sphere1->GetCenter().z - 9.71));
	...
	std::shared_ptr<Sphere> sphere6 = std::make_shared<Sphere>(3.4, Vector(0, -1 + (3.4), -4));
	sphere6->SetMaterial(mirrorM);

	std::shared_ptr<Disk> disk1 = std::make_shared<Disk>();
	disk1->SetMaterial(mirrorM);

	std::shared_ptr<Triangle> triangle1 = std::make_shared<Triangle>(Vector(-2, -1, 0), Vector(-1, -1, 0), Vector(-2, 1, 0));
	triangle1->SetMaterial(yellowM);

	std::shared_ptr<Triangle> triangle2 = std::make_shared<Triangle>(Vector(2, -1, -4), Vector(0, 3, -4), Vector(0, -1, -4));
...
std::vector<std::shared_ptr<Light>> Scene::InitLightSources()
{
	...
	Vector light1Position(-7, 6, -2);
	//Vector light1Position(1, 3, -1);
	std::shared_ptr<Light> light1 = std::make_shared<Light>(light1Position, Color(255, 255, 255), 5, Light::POINT);
