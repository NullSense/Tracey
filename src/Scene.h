...
class Scene {
 ...
 private:
  ...
  // Color, ambient (more == brighter), reflective, refractive, diffusive (more
  // == matte), special (2 == tile floor, for planes)
  ...
  const Material checkerSphereM = Material(white, 0, 0, 0, 0, 1);
  const Material checkerTriM = Material(white, 0, 0, 0, 0, 1);
  ...
  const Material transparentM =
      Material(white, 0.03, 0.001, GLOBAL_REFRACTION, 0, 0);
};  //															amb,  refl, refr, diff,
    //special
